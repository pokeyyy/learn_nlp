...
def greedy_decoder(model, enc_input, start_symbol):
    """
    贪婪解码器 (Greedy Decoder)
    贪婪解码是束搜索 (Beam Search) 在 K=1 时的特例。在推理阶段，由于我们不知道
    目标序列的真实输入，需要逐步自回归地生成目标序列的每个 token。

    工作原理:
    1. 初始化一个全零的解码器输入张量 (长度为 tgt_len)
    2. 从起始符号 S 开始，将当前生成的 token 填入下一个位置
    3. 将当前解码器输入送入模型，获取所有位置的 logits
    4. 取当前位置上概率最大的词作为下一个 token
    5. 重复步骤 2-4，直到填满整个序列

    :param model: Transformer 模型实例
    :param enc_input: 编码器输入, 形状 (batch_size, src_len)
    :param start_symbol: 起始符号的词汇索引 (本例中 tgt_vocab["S"] = 5)
    :return: 生成的目标输入序列, 形状 (batch_size, tgt_len)
    """
    # 编码器前向传播，获取上下文表示
    enc_outputs, enc_self_attns = model.encoder(enc_input)

    # 初始化解码器输入为全零张量，形状 (1, tgt_len)，batch_size=1
    ...
    # 逐步生成目标序列，每次生成一个位置
    for i in range(0, 5):
        # 将当前生成的 token 填入第 i 个位置
        dec_input[0][i] = next_symbol

        # 将当前解码器输入送入解码器，注意 dec_input 是逐步构建的
        # 前 i 个位置包含已生成的 token，后续位置仍为 0
        dec_outputs, _, _ = model.decoder(dec_input, enc_input, enc_outputs)

        # 将解码器输出投影到词汇空间: (1, tgt_len, tgt_vocab_size)
        projected = model.projection(dec_outputs)

        # 对每个位置取概率最大的词: squeeze 移除 batch 维度后取 argmax
        # prob 形状: (tgt_len,)，每个元素是该位置预测出的 token 索引
        prob = projected.squeeze(0).max(dim=-1, keepdim=False)[1]

        # 取第 i 个位置预测出的 token 作为下一个输入
