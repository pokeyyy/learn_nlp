...
def greedy_decoder(model, enc_input, start_symbol):
    """
    为简化起见，贪婪解码器是 K=1 时的束搜索。这在推理时是必要的，因为我们不知道
    目标序列的输入。因此，我们尝试逐字生成目标输入，然后将其送入 Transformer。
    参考: http://nlp.seas.harvard.edu/2018/04/03/attention.html#greedy-decoding
    :param model: Transformer 模型
    :param enc_input: 编码器输入
    :param start_symbol: 起始符号，本例中为 'S'，对应索引为 4
    :return: 目标输入序列
    """
